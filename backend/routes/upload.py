...
try:
    from backend.utils.xai import GradCAMSimulator
    from backend.model_loader import brain_tumor_model
except ImportError:
...
def preprocess_image_from_path(file_path):
    ...
    if file_name.endswith('.dcm') or file_name.endswith('.dicom'):
        ...
        if img_array.ndim == 2:
            img_array = cv2.cvtColor(img_array.astype(np.uint8), cv2.COLOR_GRAY2RGB)
        elif img_array.shape[-1] == 1:
            img_array = np.repeat(img_array, 3, axis=-1)

        img_array = cv2.resize(img_array.astype(np.uint8), (224, 224))
